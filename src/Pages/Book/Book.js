...
const errorStyle = 'text-danger text-start ms-2';
    ...
    <input className={inputStyle} style={{ outline: "none" }} placeholder={`Your Full Name`} {...register("name", { required: true })} />
      {errors.name && <span className={errorStyle}>This field is required</span>}
                
                
      <input type="email" className={inputStyle} style={{outline: "none"}} placeholder="Your Email" {...register("email", { required: true })} />      
      ...
      <input className={inputStyle} style={{outline: "none"}} placeholder="Phone" {...register("phone", { required: true , pattern: /^\(?([0-9]{5})\)?[-. ]?([0-9]{3})[-. ]?([0-9]{3})$/ })} />
        ...
        <textarea rows="8"  className={inputStyle} style={{outline: "none"}} placeholder="Message" {...register("message", { required: true })} />
